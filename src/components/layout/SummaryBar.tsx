...
export function SummaryBar() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: kols.length, stageCounts, overdueCount };
  }, [kols]);

  const stageEntries = Object.entries(stats.stageCounts) as [Stage, number][];

  return (
    <div className="flex items-center gap-4 border-b px-6 py-3 bg-background overflow-x-auto">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {stageEntries.map(([stage, count]) => (
          <Badge
            ...
            variant="outline"
            className={cn(
              'gap-1.5 text-[11px] font-normal py-0.5 px-2',
            )}
          >
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        <AlertCircle className={cn('h-3.5 w-3.5', stats.overdueCount > 0 ? 'text-overdue' : 'text-muted-foreground')} />
        <span className={cn('text-sm font-medium', stats.overdueCount > 0 ? 'text-overdue' : 'text-muted-foreground')}>
          {stats.overdueCount}
        </span>
